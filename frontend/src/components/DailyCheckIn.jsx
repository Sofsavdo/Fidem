...
export default function DailyCheckIn() {
  ...
  const claim = async () => {
    ...
    try {
      ...
      toast.success(`+${r.data.bonus.toLocaleString()} so'm · ${r.data.streak} kun ketma-ket`);
      await refresh();
      setOpen(false);
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" data-testid="daily-checkin">
      ...
      <div className="relative w-full max-w-sm bg-card rounded-3xl p-6 text-center animate-fade-up shadow-elevated">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          ...
          <span className="text-foreground font-semibold">+{status.next_bonus.toLocaleString()} so'm</span>
        </p>
        ...
        <p className="text-[11px] text-muted-foreground mt-3">7-kun streak'da +500 so'm bonus 🎁</p>
        <button
