...
export default function Verification() {
  ...
  if (!data) return <div className="p-6 text-muted-foreground">Yuklanmoqda...</div>;
    ...
    <div className="max-w-3xl mx-auto p-4 md:p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-heading font-semibold flex items-center gap-2">
          <ShieldCheck className="w-6 h-6 text-primary" /> Profil tasdiqlash
        </h1>
        <p className="text-sm text-muted-foreground mt-1">Tasdiqlangan profil ko'proq ishonchli va 3x ko'p like oladi.</p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-4 text-sm">
        <p className="font-medium mb-2">Status izohi</p>
        <ul className="space-y-1 text-muted-foreground text-xs">
          <li><Clock className="inline w-3 h-3 mr-1" /> Kutilmoqda — admin tasdig'i kutilmoqda</li>
          <li><CheckCircle2 className="inline w-3 h-3 mr-1 text-emerald-600" /> Tasdiqlandi — profil badge berildi</li>
          <li><XCircle className="inline w-3 h-3 mr-1 text-rose-600" /> Rad etildi — sabab bilan, qaytadan yuborishingiz mumkin</li>
        </ul>
      </div>
    </div>
  );
...
function VerifyCard({ kind, verified, last, onSubmit, busy }) {
  ...
  const onFile = (e) => {
    ...
    if (!f) return;
    if (f.size > 8 * 1024 * 1024) { toast.error("Maksimal 8MB"); return; }
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          ...
          {verified ? (
            ...
            <span className="text-[11px] text-muted-foreground">Tasdiqlanmagan</span>
          )}
        </div>
      </div>
      <p className="text-xs text-muted-foreground">{kind.desc}</p>
      {status === "rejected" && last?.rejection_reason && (
        <p className="text-xs text-rose-700 bg-rose-50 rounded-lg p-2">Sabab: {last.rejection_reason}</p>
      )}
      {!verified && (
        <>
          <input
          ...
          >
            ...
            {status === "pending" ? "Kutilmoqda" : status === "rejected" ? "Qayta yuklash" : "Yuklash va yuborish"}
          </button>
          {last?.proof_url && (
            <a href={last.proof_url} target="_blank" rel="noreferrer" className="text-[11px] text-primary inline-flex items-center gap-1">
              <FileText className="w-3 h-3" /> Yuklangan faylni ko'rish
            </a>
          )}
        </>
      )}
